  ...
  ast = visit_node_default(ast, (node) => {
    switch (node.type) {
      ...
      case "VariableDeclaration": {
        ...
        if ((lastDeclaration == null ? void 0 : lastDeclaration.init) && text[locEnd(lastDeclaration)] !== ";") {
...
function createParseError(error, { text }) {
  var _a;
  if (!((_a = error == null ? void 0 : error.labels) == null ? void 0 : _a[0])) {
    return error;
  }
  const {
    message,
...
function shouldForceTrailingComma(path, options2, paramsKey) {
  const { node } = path;
  return getFunctionParameters(node).length === 1 && node.type.startsWith("TS") && !node[paramsKey][0].constraint && path.parent.type === "ArrowFunctionExpression" && !(options2.filepath && options2.filepath.endsWith('.ts'));
}
