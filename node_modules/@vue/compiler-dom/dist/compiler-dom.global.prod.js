...
`,-1),e.hoists.length)){let e=[P,V,k,X,w].filter(e=>s.includes(e)).map(tQ).join(", ");n(`const { ${e} } = _Vue
`,-1)}(function(e,t){if(!e.length)return;t.pure=!0;let{push:n,newline:i}=t;i();for(let r=0;r<e.length;r++){let s=e[r];s&&(n(`const _hoisted_${r+1} = `),t2(s,t),i())}t.pure=!1})(e.hoists,t),i(),n("return ")}(e,n);let u=(c?["_ctx","_push","_parent","_attrs"]:["_ctx","_cache"]).join(", ");if(r(`function ${c?"ssrRender":"render"}(${u}) {`),o(),p&&(r("with (_ctx) {"),o(),d&&(r(`const { ${h.map(tQ).join(", ")} } = _Vue
